...
- `bonsai_addon/` is a Blender 4.2+ extension (Bonsai listener) — linted by ruff but **excluded from ty** (bpy/bonsai imports unresolvable; `tool.ty.src.include` deliberately omits it). Build the install zip with `python3 dev_utils/build_bonsai_addon.py` → `dist/materialsdb_listener.zip`. Its `insert.py` is pure ifcopenshell and CI-tested; the bpy/bonsai wiring in `__init__.py` is verified by the in-Blender harness: `pytest bonsai_addon/test -q` (pytest-blender ACTIVE — do not pass `-p no:pytest-blender` for this path; needs local Blender with Bonsai importable, local-only, CI unchanged). Conftest must stay importable without bpy: pytest loads initial conftests BEFORE pytest-blender can relaunch inside Blender. Releases publish the Blender extension repository to gh-pages (`.github/workflows/extension_repo.yml`): the tag (without `v`) becomes the manifest version; one-time setup = enable GitHub Pages from the `gh-pages` branch. The repository URL users add in Blender must point at the `index.json` file (`https://cyrilwaechter.github.io/python-materialsdb/index.json`), NOT the directory: Blender 5.2 fetches the registered URL as-is and parses it as the manifest (a directory URL yields the html listing → "invalid manifest" sync error).
